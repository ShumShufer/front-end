...
export const CourseProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadCourses = useCallback(async (params?: { schoolId?: string }) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load courses') });
    }
  }, []);
  ...
  const loadCourseById = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load course') });
    }
  }, []);
  ...
  const createCourse = useCallback(async (data: Partial<Course>) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to create course') });
      throw error;
    }
  }, []);
  ...
  const updateCourse = useCallback(async (id: string, data: Partial<Course>) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to update course') });
      throw error;
    }
  }, []);
  ...
  const deleteCourse = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to delete course') });
      throw error;
    }
  }, []);
  ...
  const loadTopics = useCallback(async (courseId: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load topics') });
    }
  }, []);

  const createTopic = useCallback(async (courseId: string, data: Partial<Topic>) => {
    dispatch({ type: 'FETCH_START' });
    try {
      await courseService.createTopic(courseId, data);
      dispatch({ type: 'ACTION_SUCCESS' });
    } catch (error: unknown) {
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to create topic') });
      throw error;
    }
  }, []);
  ...
  const updateTopic = useCallback(async (id: string, data: Partial<Topic>) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to update topic') });
      throw error;
    }
  }, []);
  ...
  const loadStudentProgress = useCallback(async (studentId: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load progress') });
    }
  }, []);
