...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const loadNotifications = useCallback(
    async (params?: { unreadOnly?: boolean }) => {
      ...
      try {
        ...
        dispatch({
          type: "FETCH_ERROR",
          payload: getErrorMessage(error, "Failed to load notifications"),
        });
      }
    },
    [],
  ...
  const markAsRead = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({
        type: "FETCH_ERROR",
        payload: getErrorMessage(error, "Failed to mark as read"),
      });
    }
  }, []);
  ...
  const markAllAsRead = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: "FETCH_ERROR",
        payload: getErrorMessage(error, "Failed to mark all as read"),
      });
    }
  }, []);
