...
export const UserProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadUsers = useCallback(async (params?: { role?: string; search?: string }) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load users') });
    }
  }, []);
  ...
  const loadUserById = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load user') });
    }
  }, []);
  ...
  const updateUser = useCallback(async (id: string, data: Partial<User>) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to update user') });
      throw error;
    }
  }, []);
  ...
  const suspendUser = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to suspend user') });
      throw error;
    }
  }, []);
  ...
  const assignRole = useCallback(async (id: string, role: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to assign role') });
      throw error;
    }
  }, []);
