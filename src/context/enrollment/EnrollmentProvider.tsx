...
import { EnrollmentContext, type EnrollmentState } from './EnrollmentContext.tsx';
...
type EnrollmentAction =
  ...
  | { type: 'FETCH_PRACTICE_REQUESTS_SUCCESS'; payload: PracticeElsewhereRequest[] }
...
function enrollmentReducer(state: EnrollmentState, action: EnrollmentAction): EnrollmentState {
  switch (action.type) {
    ...
    case 'PRACTICE_SUBMIT_SUCCESS':
      return { ...state, isLoading: false, practiceRequests: [...state.practiceRequests, action.payload] };
...
export const EnrollmentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadEnrollments = useCallback(async (params?: { schoolId?: string; studentId?: string; status?: string }) => {
    dispatch({ type: 'FETCH_START' });
    try {
      const data = await enrollmentService.getEnrollments(params);
      dispatch({ type: 'FETCH_ENROLLMENTS_SUCCESS', payload: data });
    } catch (error: unknown) {
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load enrollments') });
    }
  }, []);

  const submitApplication = useCallback(
    async (schoolId: string, mode: ApplicationMode, formResponses: Record<string, unknown>) => {
      dispatch({ type: 'FETCH_START' });
      try {
        const data = await enrollmentService.submitApplication(schoolId, mode, formResponses);
        dispatch({ type: 'SUBMIT_SUCCESS', payload: data });
      } catch (error: unknown) {
        dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to submit application') });
        throw error;
      }
    },
    [],
  ...
  const acceptApplication = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to accept application') });
      throw error;
    }
  }, []);
  ...
  const rejectApplication = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to reject application') });
      throw error;
    }
  }, []);

  const loadPracticeRequests = useCallback(async (params?: { schoolId?: string; studentId?: string }) => {
    dispatch({ type: 'FETCH_START' });
    try {
      const data = await enrollmentService.getPracticeRequests(params);
      dispatch({ type: 'FETCH_PRACTICE_REQUESTS_SUCCESS', payload: data });
    } catch (error: unknown) {
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load practice requests') });
    }
  }, []);

  const submitPracticeRequest = useCallback(async (homeSchoolId: string, hostSchoolId: string, fee: number) => {
    dispatch({ type: 'FETCH_START' });
    try {
      const data = await enrollmentService.submitPracticeRequest(homeSchoolId, hostSchoolId, fee);
      dispatch({ type: 'PRACTICE_SUBMIT_SUCCESS', payload: data });
    } catch (error: unknown) {
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to submit practice request') });
      throw error;
    }
  }, []);
  ...
  const approvePracticeRequest = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to approve practice request') });
      throw error;
    }
  }, []);
