...
type PaymentAction =
  ...
  | { type: 'INITIATE_PAYMENT_SUCCESS'; payload: { checkoutUrl: string; payment: Payment } }
...
export const PaymentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadPayments = useCallback(async (params?: { userId?: string; status?: string }) => {
    dispatch({ type: 'FETCH_START' });
    try {
      const data = await paymentService.getPayments(params);
      dispatch({ type: 'FETCH_PAYMENTS_SUCCESS', payload: data });
    } catch (error: unknown) {
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load payments') });
    }
  }, []);
  ...
  const loadPaymentById = useCallback(async (id: string) => {
    ...
    try {
      ...
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to load payment') });
    }
  }, []);

  const initiatePayment = useCallback(async (type: string, amount: number, relatedEntityId?: string) => {
    dispatch({ type: 'FETCH_START' });
    try {
      const data = await paymentService.initiatePayment(type, amount, relatedEntityId);
      dispatch({ type: 'INITIATE_PAYMENT_SUCCESS', payload: data });
    } catch (error: unknown) {
      dispatch({ type: 'FETCH_ERROR', payload: getErrorMessage(error, 'Failed to initiate payment') });
      throw error;
    }
  }, []);
