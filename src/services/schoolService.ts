...
class SchoolService implements ISchoolService {
  ...
  async getClassrooms(schoolId: string): Promise<Classroom[]> {
    return delay(400, mockClassrooms.filter((classroom) => classroom.schoolId === schoolId));
  }

  async createClassroom(schoolId: string, data: Pick<Classroom, 'name'>): Promise<Classroom> {
  ...
  async deleteClassroom(id: string): Promise<void> {
    ...
    for (let mentorIndex = mockClassroomMentors.length - 1; mentorIndex >= 0; mentorIndex -= 1) {
      if (mockClassroomMentors[mentorIndex].classroomId === id) mockClassroomMentors.splice(mentorIndex, 1);
    }
    return delay(450, undefined);
  }

  async getClassroomMentors(classroomId: string): Promise<ClassroomMentor[]> {
    return delay(350, mockClassroomMentors.filter((link) => link.classroomId === classroomId));
  }

  async assignMentor(classroomId: string, mentorId: string): Promise<ClassroomMentor> {
    const classroom = mockClassrooms.find((item) => item.id === classroomId);
    const mentor = mockUsers.find((user) => user.id === mentorId && user.role === 'MENTOR');
    if (!classroom || !mentor || mentor.schoolId !== classroom.schoolId) throw new Error('Mentor is not available for this classroom');
    const existing = mockClassroomMentors.find((link) => link.classroomId === classroomId && link.mentorId === mentorId);
  ...
  async removeMentor(classroomId: string, mentorId: string): Promise<void> {
    const index = mockClassroomMentors.findIndex((link) => link.classroomId === classroomId && link.mentorId === mentorId);
  ...
  async getStaffPosts(schoolId: string): Promise<StaffApplicationPost[]> {
    return delay(400, mockStaffPosts.filter((post) => post.schoolId === schoolId));
  }

  async createStaffPost(schoolId: string, data: Pick<StaffApplicationPost, 'role' | 'description'>): Promise<StaffApplicationPost> {
  ...
  async updateStaffPost(id: string, data: Partial<StaffApplicationPost>): Promise<StaffApplicationPost> {
  ...
  async getStaffApplications(schoolId: string): Promise<StaffApplication[]> {
    const postIds = mockStaffPosts.filter((post) => post.schoolId === schoolId).map((post) => post.id);
    return delay(400, mockStaffApplications.filter((application) => postIds.includes(application.postId)));
  }

  async updateStaffApplicationStatus(id: string, status: ApplicationStatus): Promise<StaffApplication> {
    const index = mockStaffApplications.findIndex((application) => application.id === id);
  ...
  async removeEducationHead(schoolId: string, userId: string): Promise<void> {
    const user = mockUsers.find((item) => item.id === userId && item.schoolId === schoolId);
  ...
  async getSchoolStaff(schoolId: string, roles?: Role[]): Promise<string[]> {
    return delay(350, mockUsers.filter((user) => user.schoolId === schoolId && (!roles || roles.includes(user.role))).map((user) => user.id));
  }
}
