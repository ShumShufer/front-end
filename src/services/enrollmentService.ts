...
class EnrollmentService implements IEnrollmentService {
  async getEnrollments(params?: {
    ...
    if (params?.schoolId) filtered = filtered.filter(e => e.schoolId === params.schoolId);
    if (params?.studentId) filtered = filtered.filter(e => e.studentId === params.studentId);
  ...
  async getPracticeRequests(params?: {
    ...
    if (params?.studentId) filtered = filtered.filter(r => r.studentId === params.studentId);
    if (params?.schoolId) {
      filtered = filtered.filter(
        r => r.homeSchoolId === params.schoolId || r.hostSchoolId === params.schoolId,
      );
    }
