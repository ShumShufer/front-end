...
5. **Comment with intent, not narration.** Comment _why_, not _what_. `// subtract 1 because the API is 0-indexed` is useful; `// loop through items` above a for-loop is not. Every non-trivial business rule (schedule priority lock, commission calculation, RBAC scoping) must have a comment explaining the rule.
